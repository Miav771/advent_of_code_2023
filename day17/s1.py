...
min_loss_per_approach = [
    [{approach: [2**31] * 3 for approach in approaches} for _ in range(width)]
    for _ in range(height)
]
min_loss_per_approach[0][0] = {approach: [0] * 3 for approach in approaches}
...
while queue:
    ...
    for a_y, a_x, n_y, n_x in [
        ...
        # Taking a turn
        turn_loss = min(
            min(min_loss_per_approach[c_y][c_x][approach]) + grid[n_y][n_x]
        ...
        for line_length, loss in enumerate(
            min_loss_per_approach[c_y][c_x][(a_y, a_x)][:2]
        ):
            line_length += 1
            loss += grid[n_y][n_x]
...
print(
    min(min(losses) for losses in min_loss_per_approach[height - 1][width - 1].values())
)
